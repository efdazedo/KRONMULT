...
double test_kronmult_vbatched(int const idim, int const m_array[],
                              int const n_array[], int const batchCount,
...

{
  ...
  size_t const Wcapacity_bytes =
      (Wcapacity_bytes_in == 0) ? Wcapacity_bytes_default : Wcapacity_bytes_in;
...
#pragma omp parallel for
...
#ifdef USE_GPU
  {
    ...
    {
    case 1:
      hipLaunchKernelGGL(HIP_KERNEL_NAME(kronmult1_vbatched<T>),
                         dim3(batchCount), dim3(nthreads), 0, 0, m1, n1,
                         dAparray_, dpdZarray_, dpdYarray_, dWarray_,
                         Wcapacity_bytes, batchCount);
      break;
    case 2:
      hipLaunchKernelGGL(HIP_KERNEL_NAME(kronmult2_vbatched<T>),
                         dim3(batchCount), dim3(nthreads), 0, 0, m1, n1, m2, n2,
                         dAparray_, dpdZarray_, dpdYarray_, dWarray_,
                         Wcapacity_bytes, batchCount);
      break;
    case 3:
      hipLaunchKernelGGL(HIP_KERNEL_NAME(kronmult3_vbatched<T>),
                         dim3(batchCount), dim3(nthreads), 0, 0, m1, n1, m2, n2,
                         m3, n3, dAparray_, dpdZarray_, dpdYarray_, dWarray_,
                         Wcapacity_bytes, batchCount);
      break;
    case 4:
      hipLaunchKernelGGL(HIP_KERNEL_NAME(kronmult4_vbatched<T>),
                         dim3(batchCount), dim3(nthreads), 0, 0, m1, n1, m2, n2,
                         m3, n3, m4, n4, dAparray_, dpdZarray_, dpdYarray_,
                         dWarray_, Wcapacity_bytes, batchCount);
      break;
    case 5:
      hipLaunchKernelGGL(HIP_KERNEL_NAME(kronmult5_vbatched<T>),
                         dim3(batchCount), dim3(nthreads), 0, 0, m1, n1, m2, n2,
                         m3, n3, m4, n4, m5, n5, dAparray_, dpdZarray_,
                         dpdYarray_, dWarray_, Wcapacity_bytes, batchCount);
      break;
    case 6:
      hipLaunchKernelGGL(HIP_KERNEL_NAME(kronmult6_vbatched<T>),
                         dim3(batchCount), dim3(nthreads), 0, 0, m1, n1, m2, n2,
                         m3, n3, m4, n4, m5, n5, m6, n6, dAparray_, dpdZarray_,
                         dpdYarray_, dWarray_, Wcapacity_bytes, batchCount);
      break;
    default:
      assert(false);
    ...
    if (istat != hipSuccess)
    {
      std::cout << " line " << __LINE__ << " istat = " << istat << std::endl;
    };
    assert(istat == hipSuccess);
  }
#else

  {
    ...
    {
    case 1:
      kronmult1_vbatched<T>(m1, n1, dAparray_, dpdZarray_, dpdYarray_, dWarray_,
                            Wcapacity_bytes, batchCount);
      break;

    case 2:
      kronmult2_vbatched<T>(m1, n1, m2, n2, dAparray_, dpdZarray_, dpdYarray_,
                            dWarray_, Wcapacity_bytes, batchCount);
      break;

    case 3:
      kronmult3_vbatched<T>(m1, n1, m2, n2, m3, n3, dAparray_, dpdZarray_,
                            dpdYarray_, dWarray_, Wcapacity_bytes, batchCount);
      break;

    case 4:
      kronmult4_vbatched<T>(m1, n1, m2, n2, m3, n3, m4, n4, dAparray_,
                            dpdZarray_, dpdYarray_, dWarray_, Wcapacity_bytes,
                            batchCount);
      break;

    case 5:
      kronmult5_vbatched<T>(m1, n1, m2, n2, m3, n3, m4, n4, m5, n5, dAparray_,
                            dpdZarray_, dpdYarray_, dWarray_, Wcapacity_bytes,
                            batchCount);
      break;

    case 6:
      kronmult6_vbatched<T>(m1, n1, m2, n2, m3, n3, m4, n4, m5, n5, m6, n6,
                            dAparray_, dpdZarray_, dpdYarray_, dWarray_,
                            Wcapacity_bytes, batchCount);
      break;
...
int main_func(double const tol)
{
            ...
            for (int m4 = 1; m4 <= m4max; m4++)
              for (int m5 = 1; m5 <= m5max; m5++)
                ...
                for (int m6 = 1; m6 <= m6max; m6++)
                  ...
                  double const max_abserr = test_kronmult_vbatched<T>(
                      idim, m_array, n_array, batchCount, idebug);
                  ...
                  if ((idebug >= 1) || (!isok))
                  {
                    std::cout << " idim = " << idim << " m1,n1 " << m1 << ","
                              << n1 << " m2,n2 " << m2 << "," << n2 << " m3,n3 "
                              << m3 << "," << n3 << " m4,n4 " << m4 << "," << n4
                              << " m5,n5 " << m5 << "," << n5 << " m6,n6 " << m6
                              << "," << n6 << " batchCount = " << batchCount
  ...
  if (nerrors == 0)
  {
    ...
    for (int n = 4; n <= 6; n++)
    {
      ...
      test_kronmult_vbatched<T>(idim, m_array, n_array, batchCount, idebug,
                                do_check);
    };
  };
