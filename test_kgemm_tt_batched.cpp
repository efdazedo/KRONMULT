...
template<typename T>
T test_kgemm_tt_batched( int const mm, 
                              int const nn, 
                              int const kk, 
                              ...
                              int const idebug = 1)
{
	...
	assert( ldCarray_ != nullptr );
        ...
        hipLaunchKernelGGL(HIP_KERNEL_NAME(kgemm_tt_batched<T>), dim3(batchCount), dim3(nthreads ), 0, 0,  mm,nn,kk, 
                          alpha,
...
int main_func( double const tol)
{
        ...
        for(int mm=1; mm <= mm_max; mm += inc) {
                double const max_abserr = test_kgemm_tt_batched<double>(mm,nn,kk,batchCount,idebug);
