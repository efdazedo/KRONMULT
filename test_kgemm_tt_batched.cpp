...
template<typename T>
T test_kgemm_tt_batched(int const mm, int const nn, int const kk,
                        int const batchCount, int const idebug = 1)
{
  // ----------------------------------------
  // simple program to test kgemm_tt_batched
  // ----------------------------------------
  ...
  // --------------------------------------------------
  // for testing kgemm_tt
  // C is (mm by nn), A is (kk by mm), B is (nn by kk)
  // --------------------------------------------------
  int const nrowA = kk;
  int const ncolA = mm;
...
#ifdef USE_GPU
  {
    ...
    hipLaunchKernelGGL(HIP_KERNEL_NAME(kgemm_tt_batched<T>), dim3(batchCount),
                       dim3(nthreads), 0, 0, mm, nn, kk, alpha, ddAarray_,
                       dldAarray_, ddBarray_, dldBarray_, beta, ddCarray_,
                       dldCarray_, batchCount);
...
#else
  {
    kgemm_tt_batched<T>(mm, nn, kk, alpha, ddAarray_, dldAarray_, ddBarray_,
  ...
  for (int ibatch = 0; ibatch < batchCount; ibatch++)
  {
    ...
    for (int j = 1; j <= nn; j++)
    {
      for (int i = 1; i <= mm; i++)
      {
        ...
        {
          // ---------------------------
          // note   C = transpose(A) * transpose(B)
          // ---------------------------
          cij += A(k, i) * B(j, k);
        };
      ...
      for (int nn = 1; nn <= nn_max; nn += inc)
      {
        for (int mm = 1; mm <= mm_max; mm += inc)
        {
          double const max_abserr =
              test_kgemm_tt_batched<double>(mm, nn, kk, batchCount, idebug);
  ...
  {
    {
      ...
      for (int i = 1; i <= 5; i++)
      {
        ...
        test_kgemm_tt_batched<T>(mm, nn, kk, batchCount, idebug);
        batchCount *= n;
      };
    };
