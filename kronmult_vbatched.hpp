...
kronmult_vbatched(int const m1_in, int const n1_in, int const m2_in,
                  int const n2_in, int const m3_in, int const n3_in,
                  int const m4_in, int const n4_in, int const m5_in,
                  int const n5_in, int const m6_in, int const n6_in,
                  T const *const Aarray_[], T *pX_[], T *pY_[], T *W_,
                  size_t const Wcapacity_bytes, int const batchCount_in)
//
...
//   thus
//   need temporary storage for pW_[] to be { 2*max(sizeW , sizeX) } *
//   subbatchCount
//
// Y is the output
// X is the input
...
//
{
  ...
  int const m1 = (ndim >= 1) ? m1_in : 1;
  int const n1 = (ndim >= 1) ? n1_in : 1;
  int const m2 = (ndim >= 2) ? m2_in : 1;
  int const n2 = (ndim >= 2) ? n2_in : 1;
  int const m3 = (ndim >= 3) ? m3_in : 1;
  int const n3 = (ndim >= 3) ? n3_in : 1;
  int const m4 = (ndim >= 4) ? m4_in : 1;
  int const n4 = (ndim >= 4) ? n4_in : 1;
  int const m5 = (ndim >= 5) ? m5_in : 1;
  int const n5 = (ndim >= 5) ? n5_in : 1;
  int const m6 = (ndim >= 6) ? m6_in : 1;
  int const n6 = (ndim >= 6) ? n6_in : 1;
#ifdef USE_GPU
  // -------------------------------------------
  ...
  int const subbatchCount =
      max(1, min(batchCount_in, (Wcapacity_bytes / (2 * sizeXW * sizeof(T)))));
  if (idebug >= 1)
  {
...
#pragma omp parallel for
    ...
    for (int iz = iz_start; iz <= batchCount; iz += iz_size)
    {
      ...
      {
      case 1: {
        kronmultv1(m1, n1, A1, ld1, nvec, Xp, Yp, Wp);
        break;
      }
      case 2: {
        kronmultv2(m1, n1, A1, ld1, m2, n2, A2, ld2, nvec, Xp, Yp, Wp);
        break;
      }
      case 3: {
        kronmultv3(m1, n1, A1, ld1, m2, n2, A2, ld2, m3, n3, A3, ld3, nvec, Xp,
                   Yp, Wp);
        break;
      }
      case 4: {
        kronmultv4(m1, n1, A1, ld1, m2, n2, A2, ld2, m3, n3, A3, ld3, m4, n4,
                   A4, ld4, nvec, Xp, Yp, Wp);
        break;
      }
      case 5: {
        kronmultv5(m1, n1, A1, ld1, m2, n2, A2, ld2, m3, n3, A3, ld3, m4, n4,
                   A4, ld4, m5, n5, A5, ld5, nvec, Xp, Yp, Wp);
        break;
      }
      case 6: {
        kronmultv6(m1, n1, A1, ld1, m2, n2, A2, ld2, m3, n3, A3, ld3, m4, n4,
                   A4, ld4, m5, n5, A5, ld5, m6, n6, A6, ld6, nvec, Xp, Yp, Wp);
        break;
      }
      default: {
        assert(false);
