#ifndef KGEMM_TT_BATCHED_H
#define KGEMM_TT_BATCHED_H 1
...
kgemm_tt_batched(int const mm, int const nn, int const kk, T const alpha,
                 ...
                 int const ldCarray_[], int const batchCount)
{
  ...
  for (int ibatch = iz_start; ibatch <= batchCount; ibatch += iz_size)
  {
    ...
    kgemm_tt(mm, nn, kk, alpha, A_, ldA, B_, ldB, beta, C_, ldC);
  };
}
